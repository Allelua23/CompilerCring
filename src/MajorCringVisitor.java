...
public class MajorCringVisitor extends cringBaseVisitor<Node> {
    ...
    private SymbolTable top = null; // текущая или верхняя таблица символов
    ...
    @Override
    public Node visitMainProg(cringParser.MainProgContext ctx) { return visit(ctx.block()); } // program -> block
    ...
    public Node visitBlock(cringParser.BlockContext ctx) {
        ...
        // объединение Stmt в общее выражение Seq(Stmt1, Seq(Stmt2, Seq(Stmt3, Stmt.Null)))
    ...
    public Node visitIfStatement(cringParser.IfStatementContext ctx) {
        Expr x; Stmt s1, s2;
        //x = (Expr) visit(ctx.parametrs()); // without ELSE IF in grammatic
    ...
    public Node visitWhileStatement(cringParser.WhileStatementContext ctx) {
        ...
        Stmt saved_Stmt = Stmt.Enclosing; // Сохранение охватывающей конструкции для break
        x = (Expr) visit(ctx.parametrs());
        s1 = (Stmt) visit(ctx.block());
        while_node.init(x, s1);
        Stmt.Enclosing = saved_Stmt;
        // Сброс Stmt.Enclosing
        return while_node;
    ...
    public Node visitEqualityConclusion(cringParser.EqualityConclusionContext ctx) {
        return new Reletion(ctx.op.getText(), (Expr) visit(ctx.expression(0)), (Expr) visit(ctx.expression(1)));
    }
        ...
        if (ctx.parametr().size() > 1) {
            for (int i = 1; i < ctx.parametr().size(); i++) {
                switch (ctx.pob.getText()) {
                    case "&&":
                        x = new And("&&", x, (Expr) visit(ctx.parametr(i)));
                    case "and":
                        x = new And("and", x, (Expr) visit(ctx.parametr(i)));
                    case "||":
                        x = new Or("||", x, (Expr) visit(ctx.parametr(i)));
                    case "or":
    ...
    // return: int|float|char|bool and Type in Function
    @Override
    ...
    public Node visitExpressionList(cringParser.ExpressionListContext ctx) {
        ...
        ExprList str = new ExprList(x2, x1);
        while (!e.empty())
            str = new ExprList(e.pop(), str);
    ...
    @Override
    public Node visitBreakStat(cringParser.BreakStatContext ctx) {
        Stmt savedStmt;
        return new Break();
    }
